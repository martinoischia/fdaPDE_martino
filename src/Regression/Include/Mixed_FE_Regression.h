...
protected:
	...
	void computeDOFExact_iterative_old(UInt output_indexS, UInt output_indexT, Real lambdaS, Real lambdaT);
	...
	bool stopping_criterion(UInt index, Real J, Real J_old, Real normalized_residual_norm);
...
public:
	//! A Constructor.
	MixedFERegressionBase(const InputHandler &regressionData, OptimizationData &optimizationData, UInt nnodes_) : N_(nnodes_), M_(regressionData.isMixed() ? regressionData.getNumberofUnits() : 1), regressionData_(regressionData), optimizationData_(optimizationData), _dof(optimizationData.get_DOF_matrix())
	{
		isGAMData = regressionData.getisGAM();
		isIterative = regressionData_.getFlagIterative();
	};

	MixedFERegressionBase(const std::vector<Real> &mesh_time, const InputHandler &regressionData, OptimizationData &optimizationData, UInt nnodes_) : mesh_time_(mesh_time), N_(nnodes_), M_(regressionData.getFlagParabolic() ? mesh_time.size() - 1 : mesh_time.size() - 1 + MixedSplineRegression<InputHandler>::SPLINE_DEGREE),
	...
	MatrixXr const &getDOF(void) const
	{
		if (optimizationData_.get_DOF_matrix().rows() != 0 && optimizationData_.get_DOF_matrix().cols() != 0)
	...
	*/
	...
	MatrixXi iterations__;
	MatrixXv residual_norm__;
};
...
{
public:
	MixedFERegression(const InputHandler &regressionData, OptimizationData &optimizationData, UInt nnodes_) : MixedFERegressionBase<InputHandler>(regressionData, optimizationData, nnodes_){};
	MixedFERegression(const std::vector<Real> &mesh_time, const InputHandler &regressionData, OptimizationData &optimizationData, UInt nnodes_) : MixedFERegressionBase<InputHandler>(mesh_time, regressionData, optimizationData, nnodes_){};
